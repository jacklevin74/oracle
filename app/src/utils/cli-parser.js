...
function parseCliArgs(argv) {
    ...
    const logFile = (logArg === null || logArg === void 0 ? void 0 : logArg.split('=')[1]) || null;
    // Check if we should daemonize (background) after prompt
    const shouldDaemonize = args.includes('--daemon') || args.includes('-d');
    const isForkedChild = process.env[constants_1.FORKED_ENV_VAR] === '1';
    ...
    const keyFileName = (keyFileArg === null || keyFileArg === void 0 ? void 0 : keyFileArg.split('=')[1]) || null;
...
function validateCliOptions(options) {
    const { isDryRun, walletPath, privateKeyFromEnv, useStdin, usePrompt, keyFileName, } = options;
